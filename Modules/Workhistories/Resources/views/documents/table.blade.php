<table class="table table-hover m-b-0" id="documents-table">
    ...
    <tbody>
        <tr v-for="(documents, key) in advancedSearchFilterPaginate()" :key="key">
            ...
            <td>@{{ documents.work_histories_config_documents? documents.work_histories_config_documents.name : '' }}</td>
            ...
            <td>
                <button @click="edit(documents)" data-backdrop="static" data-target="#modal-form-documents" data-toggle="modal" class="btn btn-white btn-icon btn-md" data-toggle="tooltip" data-placement="top" title="@lang('crud.edit')"><i class="fas fa-pencil-alt"></i></button>
