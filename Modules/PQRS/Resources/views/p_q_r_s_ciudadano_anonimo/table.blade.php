<div class="table-responsive">
    <table-component
        ...
        >
            ...
            <table-column show="pqr_id" label="@lang('ID')">
                <template slot-scope="row"><div>@{{ row.pqr_id }}</div></template>
            </table-column>

            {{-- <table-column show="estado" label="@lang('Estado')">
                <template slot-scope="row">
                    <div class="text-center" :class="row.estado == 'Abierto' ? 'estado_abierto' : (row.estado == 'Finalizado' && row.linea_tiempo == 'A tiempo' ? 'estado_finalizado' : (row.estado == 'Finalizado vencido justificado' ? 'estado_finalizado_vencido_justificado' : (row.linea_tiempo == 'A tiempo' ? 'estado_a_tiempo' : (row.linea_tiempo == 'Próximo a vencer' ? 'estado_proximo_vencer' : 'estado_vencido'))))">
                        @{{ row.estado }} <br />
                        <span v-if="row.linea_tiempo">(@{{ row.linea_tiempo }})</span>
                    </div>
                </template>
            </table-column> --}}
            ...
            <table-column show="nombre_ciudadano" label="@lang('Ciudadano')">
                <template slot-scope="row"><div>@{{ row.ciudadano_users ? row.ciudadano_users.name : row.nombre_ciudadano }}</div></template>
            </table-column>

            <table-column show="funcionario_destinatario" label="@lang('Destinatario')">
                <template slot-scope="row"><div>@{{ row.funcionario_destinatario }}</div></template>
            </table-column>

            <table-column show="pqr_eje_tematico" label="@lang('Eje temático')">
                <template slot-scope="row">
                    <div>@{{ row.pqr_eje_tematico ? row.pqr_eje_tematico.nombre : row.nombre_ejetematico}}</div>
                    <div v-if="row.pqr_eje_tematico">(@{{ row.pqr_eje_tematico.plazo }} @{{ row.pqr_eje_tematico.plazo_unidad }})</div>
                    <div v-else-if="row.plazo">(@{{ row.plazo }} Días)</div>
                </template>
            </table-column>

            <table-column show="created_at" label="@lang('Fecha de creación')">
                <template slot-scope="row"><div>@{{ row.created_at }}</div></template>
            </table-column>

            <table-column show="fecha_vence" label="@lang('Fecha de vencimiento')">
                <template slot-scope="row"><div>@{{ row.fecha_vence }}</div></template>
            </table-column>

            <table-column show="updated_at" label="@lang('Última modificación')">
                <template slot-scope="row"><div>@{{ row.updated_at }}</div></template>
            </table-column>
