...
use Modules\Leca\Models\DprprCloruro;
...
class EnsayoCloruroController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoCloruro.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Cloruro::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 8)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 8)->get()->last();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $cloruroGeneral = Cloruro::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoCloruro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoCloruro();
                ...
                $ensayo->temperatura = $request['temperatura'];
                ...
                $ensayo->lc_cloruro_id = $cloruroGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 8)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = Cloruro::where('lc_list_trials_id', 8)->get()->last();
                $result3 = floatval($request['resultado'])/$datosPrim->valor_esperado*100;

                if ($result3 > $result2 && $result3 < $result1) {

                    $user = Auth::user();
                    $cantidadPatrones = EnsayoCloruro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    $ensayo = new EnsayoCloruro();
                    $ensayo->volumen_muestra = $request['volumen_muestra'];
                    $ensayo->estado = 'Si';
                    $ensayo->ph = $request['ph'];
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoCloruro::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoCloruro::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoCloruro::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 8)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de cloruro";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 8)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $duplicado = EnsayoCloruro::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoCloruro::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoCloruro::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $dpr->lc_ensayo_cloruro_id = $ensayo->id;
                            $dpr->save();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de cloruro";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoCloruro $request)
    {
     ...
     * @return Response
     */
    public function getAllCloruro()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {

            $user = Auth::user();

            $allEnsayos = EnsayoCloruro::with("lcDprPrCloruros")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoCloruro::with("lcDprPrCloruros")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoCloruro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoCloruro::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoCloruro.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 8)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 8)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 8)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 8)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 8)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 8)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Cloruro::where('lc_list_trials_id', 8)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_cloruro_id = $request['id_ensayo'];
    ...
    public function getDatosPromedio($id)
    {
        ...
        $dpr = DprprCloruro::where('id_ensayo', $id)->where('tipo', 'Promedio')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprCloruro::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoCloruro::with(['lcDprPrCloruros', 'lcObservacionesDuplicadoCloruros'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoCloruro::where('id', $request['id'])->get()->last();
        
        $date = Carbon::parse( $ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse( $ensayoA->created_at)->toTimeString();

        $blanco = EnsayoCloruro::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoCloruro::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Cloruro.xlsx');
        ...
        $datosPrimarios=Cloruro::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();

        $anio = date("y", $date);
        $mes = date("m", $date);

        $spreadsheet->getActiveSheet()->setCellValue('D7', $datosPrimarios->processo ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('K8', $datosPrimarios->año ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('L8', $datosPrimarios->mes ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('E18', $datosPrimarios->limite_cuantificacion ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('G19', $datosPrimarios->concentracion_agno ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('I19', $datosPrimarios->factor_calculo ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('D20', $datosPrimarios->nombre_patron ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('D22', $datosPrimarios->valor_esperado.'mg/L' ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('D23', $datosPrimarios->documento_referencia ?? 'N/A');


        $criticalEquip=CriticalEquipment::where('lc_list_trials_id', 8)->latest()->take(4)->get();

        $letra="E";
        for ($i=0; $i < count($criticalEquip) ; $i++) { 
            $spreadsheet->getActiveSheet()->setCellValue($letra.'15', $criticalEquip[$i]->equipo_critico ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue($letra.'16', $criticalEquip[$i]->identificacion ?? 'N/A');
            $letra++;
            $letra++;
        }

        $cont=0;
        $numTotal=26+count($input);
        $totalCeldas=count($input);
        if($totalCeldas==1){

        }else{

        $spreadsheet->getActiveSheet()->insertNewRowBefore(27, $totalCeldas-1);
        ...
        for ($i=26; $i < $numTotal; $i++) { 
            ...
            $spreadsheet->getActiveSheet()->setCellValue('D'.$i, $input[$cont]['ph'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('E'.$i, $input[$cont]['temperatura'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('F'.$i, $input[$cont]['volumen_muestra'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('G'.$i, $input[$cont]['volumen_gastado'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('I'.$i, $input[$cont]['resultado'] ?? 'N/A');
            ...
            if($input[$cont]['tipo']=='Duplicado'){
                ...
                $dprpr=DprprCloruro::where('lc_ensayo_cloruro_id', $input[$cont]['id'])->get();
            ...
            $spreadsheet->getActiveSheet()->setCellValue('L'.$i, $user->name ?? 'N/A');
        ...
        if(count($arraydpr)>0){
            ...
            for ($u=0; $u < count($arraydpr) ; $u++) { 
                ...
                if($arraydpr[$u]->tipo=='Diferencia porcentual relativa' && $contDpr<4){
                    ...
                    $spreadsheet->getActiveSheet()->setCellValue($letraD.'20', $dia ?? 'N/A');
                    $letraD++;
                }else{
                    if($arraydpr[$u]->tipo=='Promedio' &&  $contPorcentaje<4){
                        $spreadsheet->getActiveSheet()->setCellValue($letraP.'22', $arraydpr[$u]->resultado ?? 'N/A');
                        $spreadsheet->getActiveSheet()->setCellValue($letraP.'20', $dia ?? 'N/A');
                        $letraP++;
                    }
                }
    ...
    public function getUltimoBlanco()
    {
      ...
      $ensayoA = EnsayoCloruro::where('tipo','Blanco')->get()->last();
    ...
    {   
        ...
        $ensayo = EnsayoCloruro::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoCloruro::where('tipo', "Duplicado")->latest()->get();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Cloruro"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Cloruro"]]];
        $data[] = ["range" => "% S std!E7", "values" => [["Cloruro"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Cloruro"]]];
        $data[] = ["range" => "%SLCM!E7", "values" => [["Cloruro"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Cloruro"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 8)->get()->last();
        ...
        $ensayo = Cloruro::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 8)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["lc_dpr_pr_cloruros"]){
                                    foreach ($item["lc_dpr_pr_cloruros"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("1IORli_6uJ26QaOkSlUXq17DepjBd4Wrpxk3AVnIJs1Q", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Cloruro", "excel", true);
    }   
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoCloruro::find($request['id']);
