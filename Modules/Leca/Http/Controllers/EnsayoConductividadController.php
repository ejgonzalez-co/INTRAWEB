...
use Modules\Leca\Models\DprprConductividad;
...
class EnsayoConductividadController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoConductividad.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Conductividad::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 34)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 34)->get()->last();
     ...
     * @version 1.0.0
     *
     * @param EnsayoConductividad $request
     *
     * @return Response
     */
    public function store(Request $request)
    {
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoConductividad::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoConductividad();
                $ensayo->estado = 'Si';
                $ensayo->temperatura_con = $request['temperatura_con'];
                $ensayo->conductividad = $request['conductividad'];
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 34)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = Conductividad::where('lc_list_trials_id', 34)->get()->last();
                $result3 = floatval($request['conductividad'])/$datosPrim->valor_esperado_dos*100;

                if ($result3 > $result2 && $result3 < $result1) {

                    $user = Auth::user();
                    $cantidadPatrones = EnsayoConductividad::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    $ensayo = new EnsayoConductividad();
                    $ensayo->estado = 'Si';
                    $ensayo->temperatura_con = $request['temperatura_con'];
                    $ensayo->conductividad = $request['conductividad'];
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoConductividad::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoConductividad::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoConductividad::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 34)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de Conductividad";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 34)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $duplicado = EnsayoConductividad::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoConductividad::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoConductividad::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $dpr->resultado = ($request['idMuestra']['conductividad'] + $request['conductividad']) / 2;
                            $dpr->lc_ensayo_conductividad_id = $ensayo->id;
                            $dpr->save();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de Conductividad";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoConductividad $request)
    {
     ...
     * @return Response
     */
    public function getAllConductividad()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();
            $allEnsayos = EnsayoConductividad::with("lcDprPrConductividad")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoConductividad::with("lcDprPrConductividad")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoConductividad::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoConductividad::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoConductividad.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 34)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 34)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 34)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 34)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 34)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 34)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Conductividad::where('lc_list_trials_id', 34)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_conductividad_id = $request['id_ensayo'];
    ...
    public function getDatosPromedio($id)
    {
        ...
        $dpr = DprprConductividad::where('id_ensayo', $id)->where('tipo', 'Promedio')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprConductividad::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoConductividad::with(['lcDprprTurbidez', 'lcObservacionesDuplicadoTurbidez'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoConductividad::where('id', $request['id'])->get()->last();

        $blanco = EnsayoConductividad::whereDate('created_at', '>=', $ensayoA->created_at)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoConductividad::whereDate('created_at', '>=', $ensayoA->created_at)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Conductividad.xlsx');
        ...
        $datosPrimarios = Conductividad::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();
        ...
        $spreadsheet->getActiveSheet()->setCellValue('I14', $datosPrimarios->fecha_ajuste_curva_uno ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('O14', $datosPrimarios->fecha_ajuste_curva_uno ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('E15', $datosPrimarios->documento_referencia_uno ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('I17', $datosPrimarios->nombre_patron_uno ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('I18', $datosPrimarios->valor_esperador_uno ?? 'N/A');


        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 34)->latest()->get();

        $spreadsheet->getActiveSheet()->setCellValue('O13', $criticalEquip[0]->equipo_critico ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('I13', $criticalEquip[1]->equipo_critico ?? 'N/A');
        ...
        if ($totalCeldas == 1) {
            ...
            $spreadsheet->getActiveSheet()->insertNewRowBefore(26, $totalCeldas);
        ...
        for ($i = 25; $i < $numTotal; $i++) {
            ...
            $spreadsheet->getActiveSheet()->mergeCells('E' . $i . ':H' . $i);
            $spreadsheet->getActiveSheet()->mergeCells('I' . $i . ':O' . $i);
            ...
            $spreadsheet->getActiveSheet()->setCellValue('C' . $i, $hora . ':' . $minutos . ':' . $segundos ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('E' . $i, $input[$cont]['consecutivo'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('I' . $i, $input[$cont]['conductividad'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('I' . $i, $input[$cont]['temperatura_con'] ?? 'N/A');


            if ($input[$cont]['tipo'] == 'Duplicado') {

                $dprpr = DprprConductividad::where('lc_ensayo_conductividad_id', $input[$cont]['id'])->get();
        ...
        if (count($arraydpr) > 0) {
            ...
            for ($u = 0; $u < count($arraydpr); $u++) {
                ...
                if ($arraydpr[$u]->tipo == 'Diferencia porcentual relativa' && $contDpr < 2) {
                    $spreadsheet->getActiveSheet()->setCellValue('E20', $arraydpr[$u]->consecutivo ?? 'N/A');
                    $spreadsheet->getActiveSheet()->setCellValue($letraM . '21', $arraydpr[$u]->resultado . '%' ?? 'N/A');
                    $spreadsheet->getActiveSheet()->setCellValue($letraM . '20', ($arraydpr[$u]->add1 + $arraydpr[$u]->add2)/2 ?? 'N/A');
                    $spreadsheet->getActiveSheet()->setCellValue($letraO . '21', $dia ?? 'N/A');
                    $spreadsheet->getActiveSheet()->setCellValue($letraP . '21', $hora ?? 'N/A');
                }
    ...
    public function getUltimoBlanco()
    {
        ...
        $ensayoA = EnsayoConductividad::where('tipo', 'Blanco')->get()->last();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoConductividad::find($request['id']);
    ...
    {   
        ...
        $ensayo = EnsayoConductividad::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoConductividad::where('tipo', "Duplicado")->latest()->get();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Conductividad"]]];
        $data[] = ["range" => "STD!E8", "values" => [["Conductividad"]]];
        $data[] = ["range" => "LCM!E8", "values" => [["Conductividad"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Conductividad"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 34)->get()->last();
        ...
        $ensayo = Conductividad::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 34)->get()->last();
        ...
        foreach ($request->data as $keya => $item) {
            ...
            if($item["tipo"]=="Blanco") {
                ...
                if($contBk<=31){
                    array_push($arrayBk, $item["conductividad"]);
                    ...
                    if($contBk<=62 && $contBk>31){
                        array_push($arrayBk2,  $item["conductividad"]);
                        $dia = date("d", strtotime($item["created_at"]));
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    //Aqui se llena el arreglo para los primeros 31 registros
                    if($contStd<=31){
                        array_push($arrayStd, $item["conductividad"]);
                        $dia = date("d", strtotime($item["created_at"]));
                        ...
                        if($contStd<=62 && $contStd>31){
                            array_push($arrayStd2, $item["conductividad"]);
                            $dia = date("d", strtotime($item["created_at"]));
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["lc_dpr_pr_conductividad"]){
                                    foreach ($item["lc_dpr_pr_conductividad"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("1JpZlKNlKG1HdC3J7U78bnvy7fEo7ZjQ7pLtYdZJEZGg", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Conductividad", "excel", true);
    }
}