...
class EnsayoFluorurosController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoFluoruro.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Fluoruros::orderBy('id', 'desc')->first();
     ...
     * @return Response
     */
    public function getAllFluoruro()
    {

        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();

            $allEnsayos = EnsayoFluoruro::with("lcDprFluoruros")->where('users_id', $user->id)->latest()->get();

        } else {
            $allEnsayos = EnsayoFluoruro::with("lcDprFluoruros")->latest()->get();
        }
    ...
    public function indexEjecutar()
    {
        ...
        $blanco = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('aprobacion_usuario', 'Si')->get()->last();

        if ($blanco) {

            $patron = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('aprobacion_usuario', 'Si')->get()->last();

            if ($patron) {

                $arrayTotal['ensayo'] = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoFluoruro.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 20)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 20)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 20)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 20)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 20)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 20)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Fluoruros::where('lc_list_trials_id', 20)->get()->last();
    ...
    public function storeRelativa(Request $request)
    {
        ...
        $dpr->id_muestra = $request['id_muestra'];
        $dpr->resultado = $request['resultado'];
        $dpr->lc_ensayo_fluoruros_id = $request['id_ensayo'];
        $dpr->save();
        return $this->sendResponse('si', trans('data_obtained_successfully'));
    }
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprFluoruro::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function exporta(Request $request)
    {
        ...
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Fluoruros.xlsx');
        ...
        $datosPrimarios = Fluoruros::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();

        $anio = date("y", $date);
        $mes = date("m", $date);

        $spreadsheet->getActiveSheet()->setCellValue('E16', $datosPrimarios->nombre_patron ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('E17', $datosPrimarios->valor_esperado.'mg/L' ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('E18', $datosPrimarios->documento_referencia ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('J7', $anio ?? 'N/A');

        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 20)->latest()->take(3)->get();

        $letra = "E";
        for ($i = 0; $i < count($criticalEquip); $i++) {

            $spreadsheet->getActiveSheet()->setCellValue($letra . '11', $criticalEquip[$i]->equipo_critico ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue($letra . '12', $criticalEquip[$i]->identificacion ?? 'N/A');
            $letra++;
            $letra++;
        }

        $cont = 0;
        $numTotal = 21 + count($input);
        $totalCeldas = count($input);
        if ($totalCeldas == 1) {

        } else {

            $spreadsheet->getActiveSheet()->insertNewRowBefore(22, $totalCeldas - 1);

        }

        $arraydpr = array();
        for ($i = 21; $i < $numTotal; $i++) {
            ...
            $spreadsheet->getActiveSheet()->setCellValue('F' . $i, $input[$cont]['mv'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('G' . $i, $input[$cont]['mg_fl'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('I' . $i, $input[$cont]['volumen_muestra'] ?? 'N/A');
            ...
            if ($input[$cont]['tipo'] == 'Duplicado') {
                ...
                $dprpr = DprFluoruro::where('lc_ensayo_fluoruros_id', $input[$cont]['id'])->get();
            ...
            if (count($arraydpr) > 0) {
                ...
                for ($u = 0; $u < count($arraydpr); $u++) {
                    ...
                    if ($arraydpr[$u]->tipo == 'Diferencia porcentual relativa' && $contDpr < 2) {
                        $spreadsheet->getActiveSheet()->setCellValue($letraD . $i, $arraydpr[$u]->resultado . '%' ?? 'N/A');
                        $spreadsheet->getActiveSheet()->setCellValue($letraD . $i, $dia ?? 'N/A');
                        $letraD++;
                    }
                    //  else {
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        $ensayo = EnsayoFluoruro::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoFluoruro::where('tipo', "Duplicado")->latest()->get();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoFluoruro::find($request['id']);
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $aluminioGeneral = Fluoruros::orderBy('id', 'desc')->first();

        if ($request['tipo'] == 'Blanco') {

            $datosBlanco = Blancos::where('lc_list_trials_id', 20)->get()->last();
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoFluoruro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoFluoruro();
                $ensayo->volumen_muestra = $request['volumen_muestra'];
                $ensayo->mv = $request['mv'];
                $ensayo->mg_fl = $request['mg_fl'];
                $ensayo->estado = 'Si';
                $ensayo->id_carta = $datosBlanco->id;
                $ensayo->aprobacion_usuario = 'Pendiente';
                $ensayo->vigencia = $Year;
                $ensayo->lc_fluoruros_id = $aluminioGeneral->id;
                $ensayo->id_muestra = '';
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 20)->get()->last();
                ...
                if ($result1 > $result2) {
                    ...
                    $user = Auth::user();
                        $cantidadPatrones = EnsayoFluoruro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();
                        ...
                        if ($patron) {
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 20)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de fluoruros";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 20)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        $total_promedio = ($request['mg_fl'] + $request['mg_fl'])/2;
                        $año = date('Y');
                        $array = str_split($año);
                        $ensayo = new EnsayoFluoruro();
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoFluoruro::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Fluoruros"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Fluoruros"]]];
        $data[] = ["range" => "% S std!E7", "values" => [["Fluoruros"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Fluoruros"]]];
        $data[] = ["range" => "%SLCM!E7", "values" => [["Fluoruros"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Fluoruros"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 20)->get()->last();
        ...
        $ensayo = Fluoruros::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 20)->get()->last();
            ...
            if($item["tipo"]=="Blanco") {
                ...
                if($contBk<=31){
                    ...
                    if($contBk<=62 && $contBk>31){
                        array_push($arrayBk2,  $item["mg_fl"]);
                        $dia = date("d", strtotime($item["created_at"]));
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    //Aqui se llena el arreglo para los primeros 31 registros
                    if($contStd<=31){
                        array_push($arrayStd, $item["mg_fl"]);
                        $dia = date("d", strtotime($item["created_at"]));
                        ...
                        if($contStd<=62 && $contStd>31){
                            array_push($arrayStd2, $item["mg_fl"]);
                            $dia = date("d", strtotime($item["created_at"]));
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["lc_dpr_fluoruros"]){
                                    foreach ($item["lc_dpr_fluoruros"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("1hvCvUxiCasbdPNk4Hx7v9hvRo3VB3Dlif19NLGV5vH8", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Fluoruros", "excel", true);
    }
