...
use Modules\Leca\Models\DprprCloro;
...
class EnsayoCloroController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoCloro.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = CloroResidual::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 16)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 16)->get()->last();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $cloroResidualGeneral = CloroResidual::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoCloro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                ...
                $ensayo->lc_cloro_id = $cloroResidualGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 16)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = CloroResidual::where('lc_list_trials_id', 16)->get()->last();
                $result3 = floatval($request['resultado'])/$datosPrim->valor_esperado*100;

                if ($result3 > $result2 && $result3 < $result1) {

                    $user = Auth::user();
                    $cantidadPatrones = EnsayoCloro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    $ensayo = new EnsayoCloro();
                    $ensayo->volumen_muestra = $request['volumen_muestra'];
                    $ensayo->estado = 'Si';
                    $ensayo->resultado_completo = $request['resultado_completo'];
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoCloro::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoCloro::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoCloro::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 16)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de cloro";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 16)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $duplicado = EnsayoCloro::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoCloro::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoCloro::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $dpr->lc_ensayo_cloro_id = $ensayo->id;
                            $dpr->save();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de cloro";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoCloro $request)
    {
     ...
     * @return Response
     */
    public function getAllCloro()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {

            $user = Auth::user();

            $allEnsayos = EnsayoCloro::with("lcDprPrCloros")->where('users_id', $user->id)->latest()->get();
        } else {
            
            $allEnsayos = EnsayoCloro::with("lcDprPrCloros")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoCloro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoCloro::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoCloro.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 16)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 16)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 16)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 16)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 16)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 16)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = CloroResidual::where('lc_list_trials_id', 16)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_cloro_id = $request['id_ensayo'];
    ...
    public function getDatosPromedio($id)
    {
        ...
        $dpr = DprprCloro::where('id_ensayo', $id)->where('tipo', 'Promedio')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprCloro::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoCloro::with(['lcDprPrCloros', 'lcObservacionesDuplicadoCloros'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoCloro::where('id', $request['id'])->get()->last();

        $blanco = EnsayoCloro::whereDate('created_at', '>=', $ensayoA->created_at)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoCloro::whereDate('created_at', '>=', $ensayoA->created_at)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Cloro.xlsx');
        ...
        $datosPrimarios=CloroResidual::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();

        $anio = date("y", $date);
        $mes = date("m", $date);


        $spreadsheet->getActiveSheet()->setCellValue('I8', $datosPrimarios->año ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('J8', $datosPrimarios->mes ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('D7', $datosPrimarios->proceso ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('A20', $datosPrimarios->limite_cuantificacion ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('C20', $datosPrimarios->nombre_patron ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('E20', $datosPrimarios->valor_esperado.'mg/L' ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('F20', $datosPrimarios->concentracion_fas ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('G20', $datosPrimarios->factor_calculo ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('D24', $datosPrimarios->documento_referencia ?? 'N/A');


        $criticalEquip=CriticalEquipment::where('lc_list_trials_id', 16)->latest()->take(4)->get();
        ...
        $cont=0;
        $numTotal=27+count($input);
        $totalCeldas=count($input);
        if($totalCeldas==1){
        }else{

                $spreadsheet->getActiveSheet()->insertNewRowBefore(28, $totalCeldas-1);

        }

        $arraydpr = array();
        for ($i=27; $i < $numTotal; $i++) { 
            ...
            if($input[$cont]['tipo']=='Duplicado'){
                $dprpr=DprPrCloro::where('lc_ensayo_cloro_id', $input[$cont]['id'])->get();
                if(count($dprpr)>0){
        ...
        if(count($arraydpr)>0){
            ...
            for ($u=0; $u < count($arraydpr) ; $u++) { 
                ...
                if($arraydpr[$u]->tipo=='Diferencia porcentual relativa' && $contDpr<3){
                    $spreadsheet->getActiveSheet()->setCellValue($letraD.'22', $arraydpr[$u]->resultado.'%' ?? 'N/A');
                    $spreadsheet->getActiveSheet()->setCellValue($letraD.'21', $dia ?? 'N/A');
                    $letraD++;
                    $letraD++;
                }else{
                    if($arraydpr[$u]->tipo=='Promedio' &&  $contPorcentaje<4){
                        $spreadsheet->getActiveSheet()->setCellValue($letraP.'23', $arraydpr[$u]->resultado ?? 'N/A');
                        $spreadsheet->getActiveSheet()->setCellValue($letraP.'21', $dia ?? 'N/A');
                        $letraP++;
                        $letraP++;
    ...
    public function getUltimoBlanco()
    {
      ...
      $ensayoA = EnsayoCloro::where('tipo','Blanco')->get()->last();
    ...
    public function getEnsayoPrincipal($id) {
        ...
        $ensayo = EnsayoCloro::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoCloro::where('tipo', "Duplicado")->latest()->get();
     ...
     * @version 1.0.0
     *
     * @param EnsayoCloroResidual $request
     *
     * @return Response
     */
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Cloro Residual"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Cloro Residual"]]];
        $data[] = ["range" => "% S std!E7", "values" => [["Cloro Residual"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Cloro Residual"]]];
        $data[] = ["range" => "%SLCM!E7", "values" => [["Cloro Residual"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Cloro Residual"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 16)->get()->last();
        ...
        $ensayo = CloroResidual::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 16)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["lc_dpr_pr_cloros"]){
                                    foreach ($item["lc_dpr_pr_cloros"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("1efZ5UTbAGFV3FnwFKpMJ6LNiLv1b1vlJdZjmaErlGk8", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Cloro Residual", "excel", true);
    }
    ...
    {
        $ensayo = EnsayoCloro::find($request['id']);
