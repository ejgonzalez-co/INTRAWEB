...
use Modules\Leca\Models\DprprHierro;
...
class EnsayoHierroController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoHierro.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Hierro::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 14)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 14)->get()->last();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $hierroGeneral = Hierro::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoHierro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoHierro();
                ...
                $ensayo->lc_hierro_id = $hierroGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 14)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = Hierro::where('lc_list_trials_id', 14)->get()->last();
                ...
                if ($result3 > $result2 && $result3 < $result1) {
                    ...
                    if ($request['tipo_patron'] == 'Patrón') {
                        $cantidadPatrones = EnsayoHierro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    } else {
                        $cantidadPatrones = EnsayoHierro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'LCM')->count();
                    }
                    ...
                    if ($request['concentracion'] > 0) {
                        $resta = $request['concentracion'] - $hierroGeneral->valor_esperado;
                        if ($resta < 0) {
                            $resta = $resta * -1;
                        }
                        $porcentajeRecuperacion = ($resta / $hierroGeneral->valor_esperado) * 100;
                    }
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoHierro::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoHierro::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoHierro::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 14)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de hierro";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 14)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoHierro::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoHierro::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoHierro::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de hierro";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoHierro $request)
    {
     ...
     * @return Response
     */
    public function getAllHierro()
    {
        
        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();

            $allEnsayos = EnsayoHierro::with("DprPrHierro")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoHierro::with("DprPrHierro")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoHierro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoHierro::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoHierro.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 14)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 14)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 14)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 14)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 14)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 14)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Hierro::where('lc_list_trials_id', 14)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_hierro_id = $request['id_ensayo'];
    ...
    public function getDatosPr($id)
    {
        ...
        $dpr = DprprHierro::where('id_ensayo', $id)->where('tipo', 'Porcentaje de recuperacion')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprHierro::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoHierro::with(['DprPrHierro', 'lcObservacionesDuplicadoHierros'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoHierro::where('id', $request['id'])->get()->last();

        $date = Carbon::parse($ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse($ensayoA->created_at)->toTimeString();

        $blanco = EnsayoHierro::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoHierro::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $datosPrimarios = Hierro::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();

        if($datosPrimarios->limite_cuantificacion == null){
            $limiteCuanti = "N/A";
        } else {
            $limiteCuanti = $datosPrimarios->limite_cuantificacion;
        }
        ...
        $spreadsheet->getActiveSheet()->setCellValue('A23', $limiteCuanti ?? 'N/A');
        ...
        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 14)->latest()->take(4)->get();
        ...
        for ($i = 25; $i < $numTotal; $i++) {
            ...
            if ($input[$cont]['tipo'] == 'Duplicado') {
                ...
                $dprpr = DprPrHierro::where('lc_ensayo_hierro_id', $input[$cont]['id'])->get();
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        // $ensayo = EnsayoHierro::where('id_muestra', $id)->where('tipo', 'Ensayo')->first();
        ...
        $ensayo = EnsayoHierro::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoHierro::where('tipo', "Duplicado")->latest()->get();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoHierro::find($request['id']);
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Hierro"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Hierro"]]];
        $data[] = ["range" => "%S std 0,1!E7", "values" => [["Hierro"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Hierro"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Hierro"]]];
        $data[] = ["range" => "Adicionado!E7", "values" => [["Hierro"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 14)->get()->last();
        ...
        $ensayo = Hierro::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 14)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["dpr_pr_hierro"]){
                                    foreach ($item["dpr_pr_hierro"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("1uPij9Xno5JD_eDJKvcM8Zxmp4wHuZbMTPKaQkamzlrk", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Hierro", "excel", true);
    }
}
