...
use Modules\Leca\Models\DprprCarbonoOrganico;
use Modules\Leca\Models\EnsayoCarbonoOrganico;

class EnsayoCarbonoOrganicoController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoCarbonoOrganico.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = CarbonoOrganico::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 26)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 26)->get()->last();
     ...
     * @version 1.0.0
     *
     * @param EnsayoCarbonoOrganico $request
     *
     * @return Response
     */
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $aluminioGeneral = CarbonoOrganico::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoCarbonoOrganico::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoCarbonoOrganico();
                ...
                $ensayo->lc_carbono_id = $aluminioGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 26)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = CarbonoOrganico::where('lc_list_trials_id', 26)->get()->last();
                ...
                if ($result3 > $result2 && $result3 < $result1) {
                    ...
                    if ($request['tipo_patron'] == 'Patrón') {
                        $cantidadPatrones = EnsayoCarbonoOrganico::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    } else {
                        $cantidadPatrones = EnsayoCarbonoOrganico::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'LCM')->count();
                    }
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 26)->get();
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 26)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
     ...
     * @return Response
     */
    public function update($id, EnsayoCarbonoOrganico $request)
    {
     ...
     * @return Response
     */
    public function getAllCarbonoOrganico()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {

            $user = Auth::user();

            $allEnsayos = EnsayoCarbonoOrganico::with("DprPrCarbonoOrganico")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoCarbonoOrganico::with("DprPrCarbonoOrganico")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoCarbonoOrganico::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoCarbonoOrganico.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 26)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 26)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 26)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 26)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 26)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 26)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = CarbonoOrganico::where('lc_list_trials_id', 26)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_carbono_organico_id = $request['id_ensayo'];
    ...
    public function getDatosPr($id)
    {
        ...
        $dpr = DprprCarbonoOrganico::where('id_ensayo', $id)->where('tipo', 'Porcentaje de recuperacion')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprCarbonoOrganico::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoCarbonoOrganico::with(['DprPrCarbonoOrganico', 'lcObservacionesDuplicadoCarbonoOrganicos'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoCarbonoOrganico::where('id', $request['id'])->get()->last();
        $date = Carbon::parse($ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse($ensayoA->created_at)->toTimeString();

        $blanco = EnsayoCarbonoOrganico::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoCarbonoOrganico::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $datosPrimarios = CarbonoOrganico::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();
        ...
        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 26)->latest()->take(4)->get();
        ...
        for ($i = 25; $i < $numTotal; $i++) {
            ...
            if ($input[$cont]['tipo'] == 'Duplicado') {
                ...
                $dprpr = DprprCarbonoOrganico::where('lc_ensayo_carbono_organico_id', $input[$cont]['id'])->get();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoCarbonoOrganico::find($request['id']);
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Carbono Organico"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Carbono Organico"]]];
        $data[] = ["range" => "%S std 0,20!E7", "values" => [["Carbono Organico"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Carbono Organico"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Carbono Organico"]]];
        $data[] = ["range" => "Adicionado!E7", "values" => [["Carbono Organico"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 26)->get()->last();
        ...
        $ensayo = CarbonoOrganico::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 26)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["dpr_pr_carbono_organico"]){
                                    foreach ($item["dpr_pr_carbono_organico"] as $value){
        ...
        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Carbono Organico", "excel", true);
    }
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        $ensayo = EnsayoCarbonoOrganico::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoCarbonoOrganico::where('tipo', "Duplicado")->latest()->get();
