...
class EnsayoEscherichiaController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoEscherichia.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = EscherichiaColi::orderBy('id', 'desc')->first();
    ...
    public function getDatosSiembra()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {
            ...
            $allEnsayos = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Ensayo')->where('ensayo', 'Escherichia')->where('estado', 'Pendiente')->latest()->get();

        } else {
            $allEnsayos = EnsayoMicro::where('tipo', 'Ensayo')->where('ensayo', 'Escherichia')->where('estado', 'Pendiente')->first();
        }
    ...
    public function update($id, Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $coliformeGeneral = EscherichiaColi::orderBy('id', 'desc')->first();
        $user = Auth::user();

        $blanco = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();
        ...
        if ($blanco) {
            ...
            if ($blanco) {
                ...
                $arrayTotal['ensayo'] = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->get()->count();
                ...
                if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                    ...
                    $centinela = false;
                    $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 22)->get();
                    ...
                    if ($centinela == true) {
                        ...
                        $history->observation = "Se realizo el analisis de la fórmula de escherichia";
                    ...
                    $buscaEnsayo = Ensayo::where('lc_list_trials_id', 22)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                    $año = date('Y');
                    $array = str_split($año);

                    $ensayo = EnsayoMicro::where('id_muestra', $id)->where('estado', 'Pendiente')->first();

                    $ensayo = new EnsayoMicro();
                    $ensayo->estado = 'No aplica';
                    $ensayo->ensayo = 'Escherichia';
                    ...
                    $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_sample_taking_id', $request['idMuestra']['id'])->where('estado', 'Activo')->where('lc_list_trials_id', 22)->first();

                    $muestras = new TendidoMuestras();
     ...
     * @return Response
     */
    public function getAllEscherichia()
    {

        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();
            
            $allEnsayos = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->latest()->get();

            // $allEnsayos = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('tipo', 'Ensayo')->where('estado', "!=", 'Pendiente')->orWhere('tipo', 'Blanco')->orWhere('tipo', 'Duplicado')->latest()->get();

        } else {
            // $allEnsayos = EnsayoMicro::where('tipo', 'Ensayo')->where('ensayo', 'Escherichia')->where('estado', "!=", 'Pendiente')->orWhere('tipo', 'Blanco')->orWhere('tipo', 'Duplicado')->latest()->get();
            $allEnsayos = EnsayoMicro::where('ensayo', 'Escherichia')->latest()->get();
        }
    ...
    public function indexEjecutar()
    {
        ...
        $blanco = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('ensayo', 'Escherichia')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('aprobacion_usuario', 'Si')->get()->last();

        if ($blanco) {

            $patron = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('aprobacion_usuario', 'Si')->get()->last();
            ...
            if ($blanco) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoEscherichia.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 22)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 22)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 22)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 22)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 22)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = EscherichiaColi::where('lc_list_trials_id', 22)->get()->last();
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('ensayo', 'Escherichia')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('ensayo', 'Escherichia')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
            ...
            if ($patron != null) {
                ...
                if ($patron->aprobacion_usuario == 'Si') {
                    $arrayTotal['ensayo'] = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                } else {
    ...
    public function store(Request $request)
    {
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            $datosBlanco = Blancos::where('lc_list_trials_id', 22)->get()->last();
            $result1 = floatval($datosBlanco->lcm);
            $result2 = floatval($request['resultado_general']);

            if ($result1 >= $result2) {
                $user = Auth::user();
                $cantidadBlancos = EnsayoMicro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->where('ensayo', 'Escherichia')->count();
                $ensayo = new EnsayoMicro();
                $ensayo->estado = 'Si';
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoMicro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('ensayo', 'Escherichia')->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoMicro();
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('ensayo', 'Escherichia')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Escherichia')->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de escherichia";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "E. Coli!B7", "values" => [["E. coli"]]];
        ...
        $data[] = ["range" => "E. Coli!V8", "values" => [[$mes]]];
        $data[] = ["range" => "E. Coli!V7", "values" => [[$anio]]];
        ...
        if(count($arrayBk)>0){
            $data[] = ["range" => "E. Coli!B28", "values" => [$arrayBk]];
            $data[] = ["range" => "E. Coli!B29", "values" => [$arrayBkF]];
            $data[] = ["range" => "E. Coli!B30", "values" => [$arrayBkC]];
        }

        if(count($arrayBk2)>0){
            $data[] = ["range" => "E. Coli!B33", "values" => [$arrayBk2]];
            $data[] = ["range" => "E. Coli!B34", "values" => [$arrayBk2F]];
            $data[] = ["range" => "E. Coli!B35", "values" => [$arrayBk2C]];
        }

        $fileIdCopy = $e->editFileExcelBatch("1GHKOiez1RCcWdtUmL4oiDtNX1gUSQrTgVxfItqfdxXI", $data, true);
    ...
    public function getIp(Request $request)
    {
        ...
        //Este es el titular del ensayo
        $data[] = ["range" => "E. Coli IP!B7", "values" => [["E. coli"]]];
        
        $fechaActual = date('d-m-Y');
        $anio = date("y", strtotime($fechaActual));
        $mes = date("m", strtotime($fechaActual));

        $data[] = ["range" => "E. Coli IP!X8", "values" => [[$mes]]];
        $data[] = ["range" => "E. Coli IP!X7", "values" => [[$anio]]];
        ...
        foreach ($request->data as $keya => $item){ 
            ...
            if($item["tipo"]=="Duplicado") {
                ...
                if($contador == 0) {
                    ...
                    $contDpr++;
                    if($contDpr<=31){
                        $infoEnsayo = EnsayoMicro::where('consecutivo', $item['consecutivo_ensayo'])->where('tipo', 'Ensayo')->where('ensayo', 'Escherichia')->get()->first();
                ...
                $contador++;
                    ...
                    if (!(in_array($item['consecutivo_ensayo']." / ".$item["users_id"], $arrayDprC) || in_array($item['consecutivo_ensayo']." / ".$item["users_id"], $arrayDprC2))) {
                        ...
                        $contDpr++;
                        if($contDpr<=31){
                            $infoEnsayo = EnsayoMicro::where('consecutivo_ensayo', $item['consecutivo_ensayo'])->where('tipo', 'Ensayo')->where('ensayo', 'Escherichia')->get()->first();
                            array_push($arrayDprC, $item["consecutivo_ensayo"]." / ".$item["users_id"]);
                            $dia = date("d", strtotime($item["created_at"]));
                            array_push($arrayDprF, $dia);
                            array_push($arrayDprA, $infoEnsayo['resultado_general']);
        ...
        if(count($arrayDprA)>0){
            $data[] = ["range" => "E. Coli IP!B28", "values" => [$arrayDprA]];
            $data[] = ["range" => "E. Coli IP!B29", "values" => [$arrayDprB]];
            $data[] = ["range" => "E. Coli IP!B30", "values" => [$arrayDprF]];
            $data[] = ["range" => "E. Coli IP!B31", "values" => [$arrayDprC]];
        }

        if(count($arrayDprA2)>0){
            $data[] = ["range" => "E. Coli IP!B34", "values" => [$arrayDprA2]];
            $data[] = ["range" => "E. Coli IP!B35", "values" => [$arrayDprB2]];
            $data[] = ["range" => "E. Coli IP!B36", "values" => [$arrayDprF2]];
            $data[] = ["range" => "E. Coli IP!B37", "values" => [$arrayDprC2]];
        }

        $fileIdCopy = $e->editFileExcelBatch("1h5uZ1gWiJW0Cn6taxDo3MnDQ9Geoi3d1WSDjt7Zs_8s", $data, true);
