...
use Modules\Leca\Models\DprprNitritos;
...
class EnsayoNitritosController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoNitritos.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Nitritos::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 10)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 10)->get()->last();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $nitritosGeneral = Nitritos::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoNitritos::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoNitritos();
                ...
                $ensayo->lc_nitritos_id = $nitritosGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 10)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);

                $datosPrim = Nitritos::where('lc_list_trials_id', 10)->get()->last();
                $result3 = floatval($request['concentracion'])/$datosPrim->valor_esperado*100;

                // $consulPesperado = Nitritos::get()->last();

                // $resulFinal = $request['concentracion']*$consulPesperado->valor_esperado;
                ...
                if ($result3 > $result2 && $result3 < $result1) {
                    ...
                    if ($request['tipo_patron'] == 'Patrón') {
                        $cantidadPatrones = EnsayoNitritos::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    } else {
                        $cantidadPatrones = EnsayoNitritos::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'LCM')->count();
                    }
                    ...
                    if ($request['concentracion'] > 0) {
                        $resta = $request['concentracion'] - $nitritosGeneral->valor_esperado;
                        if ($resta < 0) {
                            $resta = $resta * -1;
                        }
                        $porcentajeRecuperacion = ($resta / $nitritosGeneral->valor_esperado) * 100;
                    }
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoNitritos::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoNitritos::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoNitritos::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 10)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de nitritos";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 10)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoNitritos::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoNitritos::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoNitritos::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de nitritos";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoNitritos $request)
    {
     ...
     * @return Response
     */
    public function getAllNitritos()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {

            $user = Auth::user();

            $allEnsayos = EnsayoNitritos::with("DprPrNitritos")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoNitritos::with("DprPrNitritos")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoNitritos::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoNitritos::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoNitritos.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 10)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 10)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 10)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 10)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 10)->get()->last();
    ...
    {
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 10)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Nitritos::where('lc_list_trials_id', 10)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_nitritos_id = $request['id_ensayo'];
    ...
    public function getDatosPr($id)
    {
        ...
        $dpr = DprprNitritos::where('id_ensayo', $id)->where('tipo', 'Porcentaje de recuperacion')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprNitritos::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoNitritos::with(['DprprNitritos', 'lcObservacionesDuplicadoNitritos'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoNitritos::where('id', $request['id'])->get()->last();

        $date = Carbon::parse($ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse($ensayoA->created_at)->toTimeString();

        $blanco = EnsayoNitritos::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoNitritos::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Nitritos.xlsx');
        ...
        $datosPrimarios = Nitritos::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();

        $anio = date("y", $date);
        $mes = date("m", $date);

        $spreadsheet->getActiveSheet()->setCellValue('G11', 'K(pendiente):'.$datosPrimarios->k_pendiente ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('I11', 'K(intercepto):'.$datosPrimarios->b_intercepto ?? 'N/A');
        ...
        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 10)->latest()->take(4)->get();
        ...
        for ($i = 25; $i < $numTotal; $i++) {
            ...
            if ($input[$cont]['tipo'] == 'Duplicado') {
                ...
                $dprpr = DprPrNitritos::where('lc_ensayo_nitritos_id', $input[$cont]['id'])->get();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoNitritos::find($request['id']);
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        $ensayo = EnsayoNitritos::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoNitritos::where('tipo', "Duplicado")->latest()->get();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Nitritos"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Nitritos"]]];
        $data[] = ["range" => "%S std 0,1!E7", "values" => [["Nitritos"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Nitritos"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Nitritos"]]];
        $data[] = ["range" => "Adicionado!E7", "values" => [["Nitritos"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 10)->get()->last();
        ...
        $ensayo = Nitritos::orderBy('id', 'desc')->first();

        $data[] = ["range" => "STD!P8", "values" => [[$mes]]];
        $data[] = ["range" => "STD!V8", "values" => [[$anio]]];
        $data[] = ["range" => "STD!E8", "values" => [[$ensayo->valor_esperado]]];

        $data[] = ["range" => "%S std 0,1!P8", "values" => [[$mes]]];
        $data[] = ["range" => "%S std 0,1!V8", "values" => [[$anio]]];
        $data[] = ["range" => "%S std 0,1!E8", "values" => [[$ensayo->valor_esperado]]];
        
        
        $patron = Patron::where('lc_list_trials_id', 10)->get()->last();
        ...
        $data[] = ["range" => "%S std 0,1!AJ14", "values" => [[$patron->n_ant_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AJ15", "values" => [[$patron->x_ant_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AJ16", "values" => [[$patron->s_ant_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AI17", "values" => [[$patron->lcs_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AI18", "values" => [[$patron->lci_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AI19", "values" => [[$patron->las_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AI20", "values" => [[$patron->lai_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AI21", "values" => [[$patron->x_mas_porc_std]]];
        $data[] = ["range" => "%S std 0,1!AI22", "values" => [[$patron->x_menos_porc_std]]];
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["dpr_pr_nitritos"]){
                                    foreach ($item["dpr_pr_nitritos"] as $value){
        ...
        if(count($arrayStd)>0){
            ...
            $data[] = ["range" => "%S std 0,1!B34", "values" => [$arrayStd]];
            $data[] = ["range" => "%S std 0,1!B35", "values" => [$arrayStdF]];
            $data[] = ["range" => "%S std 0,1!B36", "values" => [$arrayStdC]];
        }
        ...
        if(count($arrayStd2)>0){
            ...
            $data[] = ["range" => "%S std 0,1!B39", "values" => [$arrayStd2]];
            $data[] = ["range" => "%S std 0,1!B40", "values" => [$arrayStd2F]];
            $data[] = ["range" => "%S std 0,1!B41", "values" => [$arrayStd2C]];
        }
        ...
        $fileIdCopy = $e->editFileExcelBatch("1SMW7vGet748wjxlDWgJYfBpelbwIsTXp4dlFwaVuK90", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Nitritos", "excel", true);
    }
