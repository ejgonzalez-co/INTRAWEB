...
use Modules\Leca\Models\DprprFosfato;
...
class EnsayoFosfatoController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoFosfato.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Fosfatos::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 11)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 11)->get()->last();
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        $ensayo = EnsayoFosfato::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoFosfato::where('tipo', "Duplicado")->latest()->get();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $fosfatoGeneral = Fosfatos::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoFosfato::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoFosfato();
                ...
                $ensayo->lc_fosfato_id = $fosfatoGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 11)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = Fosfatos::where('lc_list_trials_id', 11)->get()->last();
                ...
                if ($result3 > $result2 && $result3 < $result1) {
                    ...
                    if ($request['tipo_patron'] == 'Patrón') {
                        $cantidadPatrones = EnsayoFosfato::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    } else {
                        $cantidadPatrones = EnsayoFosfato::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'LCM')->count();
                    }
                    ...
                    if ($request['concentracion'] > 0) {
                        $resta = $request['concentracion'] - $fosfatoGeneral->valor_esperado;
                        if ($resta < 0) {
                            $resta = $resta * -1;
                        }
                        $porcentajeRecuperacion = ($resta / $fosfatoGeneral->valor_esperado) * 100;
                    }
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoFosfato::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoFosfato::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoFosfato::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 11)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de fosfato";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 11)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoFosfato::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoFosfato::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoFosfato::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de fosfato";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoFosfato $request)
    {
     ...
     * @return Response
     */
    public function getAllFosfato()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {

            $user = Auth::user();

            $allEnsayos = EnsayoFosfato::with("DprprFosfato")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoFosfato::with("DprprFosfato")->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoFosfato::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoFosfato::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoFosfato.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 11)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 11)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 11)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 11)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 11)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 11)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Fosfatos::where('lc_list_trials_id', 11)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_fosfato_id = $request['id_ensayo'];
    ...
    public function getDatosPr($id)
    {
        ...
        $dpr = DprprFosfato::where('id_ensayo', $id)->where('tipo', 'Porcentaje de recuperacion')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprFosfato::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoFosfato::with(['DprprFosfato', 'ObservacionesDuplicadoFosfatos'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoFosfato::where('id', $request['id'])->get()->last();

        $date = Carbon::parse($ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse($ensayoA->created_at)->toTimeString();

        $blanco = EnsayoFosfato::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoFosfato::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $datosPrimarios = Fosfatos::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();
        ...
        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 11)->latest()->take(4)->get();
        ...
        for ($i = 25; $i < $numTotal; $i++) {
            ...
            if ($input[$cont]['tipo'] == 'Duplicado') {
                ...
                $dprpr = DprPrFosfato::where('lc_ensayo_fosfato_id', $input[$cont]['id'])->get();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoFosfato::find($request['id']);
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Fosfato"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Fosfato"]]];
        $data[] = ["range" => "%S std 0,20!E7", "values" => [["Fosfato"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Fosfato"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Fosfato"]]];
        $data[] = ["range" => "Adicionado!E7", "values" => [["Fosfato"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 11)->get()->last();
        ...
        $ensayo = Fosfatos::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 11)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["dprpr_fosfato"]){
                                    foreach ($item["dprpr_fosfato"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("165p4fxI1nYM1f7dC-2oJ28iiS2rAPCgj2Y2BcJksh6o", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Fosfato", "excel", true);
    }
}