...
class EnsayoMercurioController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoMercurio.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Mercurio::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 29)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 29)->get()->last();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $espectroGeneral = Mercurio::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoEspectro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->where('Ensayo', 'Mercurio')->count();
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoEspectro::where('users_id', $user->id)->where('Ensayo', 'Mercurio')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoEspectro();
                $ensayo->volumen = $request['volumen'];
                $ensayo->curva = $request['curva'];
                $ensayo->estado = 'No';
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 29)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = Mercurio::where('lc_list_trials_id', 29)->get()->last();
                ...
                if ($result3 > $result2 && $result3 < $result1) {
                    ...
                    if ($request['tipo_patron'] == 'Patrón') {
                        $cantidadPatrones = EnsayoEspectro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->where('Ensayo', 'Mercurio')->count();
                    } else {
                        $cantidadPatrones = EnsayoEspectro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'LCM')->where('Ensayo', 'Mercurio')->count();
                    }
                    ...
                    if ($request['tipo_patron'] == 'Patrón') {
                        $cantidadPatrones = EnsayoEspectro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('Ensayo', 'Mercurio')->where('tipo', 'Patrón')->count();
                    } else {
                        $cantidadPatrones = EnsayoEspectro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('Ensayo', 'Mercurio')->where('tipo', 'LCM')->count();
                    }
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoEspectro::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('Ensayo', 'Mercurio')->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoEspectro::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('Ensayo', 'Mercurio')->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoEspectro::where('users_id', $user->id)->where('Ensayo', 'Mercurio')->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 29)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de mercurio";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 29)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoEspectro::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('Ensayo', 'Mercurio')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de mercurio";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function getAllMercurio()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();

            $allEnsayos = EnsayoEspectro::with("lcDprPrEspectros")->where('users_id', $user->id)->where('ensayo', 'Mercurio')->latest()->get();
        } else {
            $allEnsayos = EnsayoEspectro::with("lcDprPrEspectros")->where('ensayo', 'Mercurio')->latest()->get();
        }
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoEspectro::where('users_id', $user->id)->where('ensayo', 'Mercurio')->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoEspectro::where('users_id', $user->id)->where('ensayo', 'Mercurio')->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
    ...
    public function indexEjecutar()
    {
        ...
        $blanco = EnsayoEspectro::where('users_id', $user->id)->where('ensayo', 'Mercurio')->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

        if ($blanco) {

            $patron = EnsayoEspectro::where('users_id', $user->id)->where('ensayo', 'Mercurio')->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoMercurio.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 29)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 29)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 29)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 29)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 29)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 29)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Mercurio::where('lc_list_trials_id', 29)->get()->last();
    ...
    public function getDatosPr($id)
    {
        ...
        $dpr = DprprEspectro::where('id_ensayo', $id)->where('tipo', 'Porcentaje de recuperacion')->where('ensayo', 'Mercurio')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprEspectro::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->where('ensayo', 'Mercurio')->get();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoEspectro::where('id', $request['id'])->where('ensayo', 'Mercurio')->get()->last();

        $date = Carbon::parse($ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse($ensayoA->created_at)->toTimeString();

        $blanco = EnsayoEspectro::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('ensayo', 'Mercurio')->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoEspectro::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('ensayo', 'Mercurio')->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $datosPrimarios = Mercurio::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();
        ...
        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 29)->latest()->take(4)->get();
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        $ensayo = EnsayoEspectro::where('id_muestra', $id)->where('ensayo', 'Mercurio')->where('tipo', 'Ensayo')->first();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Mercurio"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Mercurio"]]];
        $data[] = ["range" => "%S std 0,20!E7", "values" => [["Mercurio"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Mercurio"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Mercurio"]]];
        $data[] = ["range" => "Adicionado!E7", "values" => [["Mercurio"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 29)->get()->last();
        ...
        $ensayo = Mercurio::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 29)->get()->last();
        ...
        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Mercurio", "excel", true);
    }
}