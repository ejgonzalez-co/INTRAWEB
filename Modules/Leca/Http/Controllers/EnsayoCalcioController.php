...
use Modules\Leca\Models\DprprCalcio;
...
class EnsayoCalcioController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoCalcio.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = Calcio::orderBy('id', 'desc')->first();
    ...
    public function getCartaDatosBlanco()
    {
        ...
        $datosBlanco = Blancos::where('lc_list_trials_id', 12)->get()->last();
    ...
    public function getCartaDatosPatron()
    {
        ...
        $datosBlanco = Patron::where('lc_list_trials_id', 12)->get()->last();
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $calcioGeneral = Calcio::orderBy('id', 'desc')->first();
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoCalcio::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoCalcio();
                ...
                $ensayo->lc_calcio_id = $calcioGeneral->id;
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 12)->get()->last();
                $result1 = floatval($datosPatron->lcs);
                $result2 = floatval($datosPatron->lci);
                $datosPrim = Calcio::where('lc_list_trials_id', 12)->get()->last();
                $result3 = floatval($request['resultado'])/$datosPrim->valor_esperado*100;

                if ($result3 > $result2 && $result3 < $result1) {

                    $user = Auth::user();
                    $cantidadPatrones = EnsayoCalcio::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                    $ensayo = new EnsayoCalcio();
                    $ensayo->volumen_muestra = $request['volumen_muestra'];
                    $ensayo->estado = 'Si';
                    $ensayo->resultado_completo = $request['resultado_completo'];
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoCalcio::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco) {

                        $patron = EnsayoCalcio::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('estado', 'Si')->get()->last();

                        if ($patron) {

                            $arrayTotal['ensayo'] = EnsayoCalcio::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 12)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de calcio";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 12)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $duplicado = EnsayoCalcio::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoCalcio::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoCalcio::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            ...
                            $dpr->lc_ensayo_calcio_id = $ensayo->id;
                            $dpr->save();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de calcio";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @return Response
     */
    public function update($id, EnsayoCalcio $request)
    {
     ...
     * @return Response
     */
    public function getAllCalcio()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();

            $allEnsayos = EnsayoCalcio::with("lcDprPrCalcios")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoCalcio::with("lcDprPrCalcios")->latest()->get();
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoCalcio::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoCalcio::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
        ...
        if ($blanco) {
            ...
            if ($patron) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoCalcio.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 12)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 12)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 12)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 12)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 12)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 12)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = Calcio::where('lc_list_trials_id', 12)->get()->last();
    ...
    public function storeDpr(Request $request)
    {
        ...
        $dpr->lc_ensayo_calcio_id = $request['id_ensayo'];
    ...
    public function storeRelativa(Request $request)
    {
        ...
        $dpr->tipo = $request['tipo'];
        $dpr->consecutivo = $request['consecutivo'];
        $dpr->resultado_completo = number_format($request['resultado_completo'], $request['cantidadDecimales']);
        $dpr->user_name = $user->name;
        $dpr->user_id = $user->id;
        // $dpr->volumen_adicionado = $request['volumenAdicionado'];
        // $dpr->volumen_muestra = $request['volumenMuestra'];
    ...
    public function getDatosPromedio($id)
    {
        ...
        $dpr = DprprCalcio::where('id_ensayo', $id)->where('tipo', 'Promedio')->get();
    ...
    public function getDatosRelativa($id)
    {
        ...
        $dpr = DprprCalcio::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoCalcio::with(['lcDprPrCalcios', 'lcObservacionesDuplicadoCalcios'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoCalcio::where('id', $request['id'])->get()->last();

        $blanco = EnsayoCalcio::whereDate('created_at', '>=', $ensayoA->created_at)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoCalcio::whereDate('created_at', '>=', $ensayoA->created_at)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    {
        $input = $request['data'];
        $fecha = $input[0]['created_at'];
        $dateToday = date('m-d-Y h:i:s');  

        $input=array_reverse($input);
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Calcio.xlsx');
        ...
        $datosPrimarios=Calcio::whereDate('created_at', '>=', $fecha)->get()->last();
        ...
        $spreadsheet->getActiveSheet()->setCellValue('G18', $datosPrimarios->concentracion ?? 'N/A');
        $spreadsheet->getActiveSheet()->setCellValue('I18', $datosPrimarios->factor_calculo ?? 'N/A');
        ...
        $criticalEquip=CriticalEquipment::where('lc_list_trials_id', 12)->latest()->take(4)->get();
        ...
        for ($i=25; $i < $numTotal; $i++) { 
            ...
            $spreadsheet->getActiveSheet()->setCellValue('G'.$i, $input[$cont]['volumen_titulacion'] ?? 'N/A');
            ...
            if($input[$cont]['tipo']=='Duplicado'){
                ...
                $dprpr=DprPrCalcio::where('lc_ensayo_calcio_id', $input[$cont]['id'])->get();
            ...
            $spreadsheet->getActiveSheet()->setCellValue('K'.$i, $user->name ?? 'N/A');
            $cont++;
        }
    ...
    public function getUltimoBlanco()
    {
      ...
      $ensayoA = EnsayoCalcio::where('tipo','Blanco')->get()->last();
    ...
    {   
        ...
        $ensayo = EnsayoCalcio::where('id', $dprs)->first();
        if($ensayo->tipo != "Duplicado" && $ensayo->tipo != "Ensayo"){
            $ensayo = EnsayoCalcio::where('tipo', "Duplicado")->latest()->get();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "BK!B7", "values" => [["Calcio"]]];
        $data[] = ["range" => "STD!E7", "values" => [["Calcio"]]];
        $data[] = ["range" => "% S std!E7", "values" => [["Calcio"]]];
        $data[] = ["range" => "LCM!E7", "values" => [["Calcio"]]];
        $data[] = ["range" => "%SLCM!E7", "values" => [["Calcio"]]];
        $data[] = ["range" => "DPR!B7", "values" => [["Calcio"]]];
        ...
        $blanco = Blancos::where('lc_list_trials_id', 12)->get()->last();
        ...
        $ensayo = Calcio::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 12)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["lc_dpr_pr_calcios"]){
                                    foreach ($item["lc_dpr_pr_calcios"] as $value){
        ...
        $fileIdCopy = $e->editFileExcelBatch("13Eez-iGOTXBXseCGuzgw2_J9CwtzUFv9LFCy4zL9HII", $data, true);

        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Calcio", "excel", true);
    }
    ...
    {
        $ensayo = EnsayoCalcio::find($request['id']);
