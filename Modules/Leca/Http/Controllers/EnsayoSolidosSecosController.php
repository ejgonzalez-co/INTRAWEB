...
use Modules\Leca\Models\DprPrSolidosSeco;
...
use Modules\Leca\Models\EnsayoSolidosSecos;
...
class EnsayoSolidosSecosController extends AppBaseController
{
    ...
    public function index()
    {
        ...
        return view('leca::EnsayoSolidosSecos.index');
    ...
    public function getDatosBlanco()
    {
        ...
        $datosBlanco = SolidosSecos::orderBy('id', 'desc')->first();
     ...
     * @return Response
     */
    public function getAllSecos()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();

            $allEnsayos = EnsayoSolidosSecos::with("lcDprPrSolidosSecos")->where('users_id', $user->id)->latest()->get();
        } else {
            $allEnsayos = EnsayoSolidosSecos::with("lcDprPrSolidosSecos")->latest()->get();
        }
    ...
    public function indexEjecutar()
    {
        ...
        $blanco = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('aprobacion_usuario', 'Si')->get()->last();

        if ($blanco) {

            $patron = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('aprobacion_usuario', 'Si')->get()->last();

            if ($patron) {

                $arrayTotal['ensayo'] = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoSolidosSecos.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 25)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 25)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 25)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 25)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 25)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 25)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = SolidosSecos::where('lc_list_trials_id', 25)->get()->last();
    ...
    public function storeRelativa(Request $request)
    {
        ...
        $dpr->id_muestra = $request['id_muestra'];
        $dpr->resultado = $request['resultado'];
        $dpr->lc_ensayo_solidos_secos_id  = $request['id_ensayo'];
        $dpr->save();
        return $this->sendResponse('si', trans('data_obtained_successfully'));
    }
    ...
    {
        $dpr = DprPrSolidosSeco::where('id_ensayo', $id)->where('tipo', 'Diferencia porcentual relativa')->get();
    ...
    public function getAllShowEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoSolidosSecos::with(['lcDprPrSolidosSecos', 'lcObservacionesDuplicadoSolidosSecos'])->where('id_muestra', $idEnsayo)->get();
        $ensayo->toArray();
    ...
    public function getAllShowPatronBlanco(Request $request)
    {
        ...
        $ensayoA = EnsayoSolidosSecos::where('id', $request['id'])->get()->last();

        $date = Carbon::parse($ensayoA->created_at)->format('Y-m-d');
        $time = Carbon::parse($ensayoA->created_at)->toTimeString();

        $blanco = EnsayoSolidosSecos::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Blanco')->where('estado', 'Si')->get()->last();
        $patron = EnsayoSolidosSecos::whereDate('created_at', '<=', $date)->whereTime('created_at', '<', $time)->where('tipo', 'Patrón')->where('estado', 'Si')->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $datosPrimarios = SolidosSecos::whereDate('created_at', '<=', $input[0]['created_at'])->get()->last();

        $anio = date("y", $date);
        $mes = date("m", $date);

        $spreadsheet->getActiveSheet()->setCellValue('O14', $datosPrimarios->documento_referencia_uno ?? 'N/A');
        ...
        $criticalEquip = CriticalEquipment::where('lc_list_trials_id', 25)->latest()->take(2)->get();
        ...
        for ($i = 19; $i < $numTotal; $i++) {
            ...
            $spreadsheet->getActiveSheet()->setCellValue('O' . $i, $input[$cont]['campo_a'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('P' . $i, $input[$cont]['campo_b'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('Q' . $i, $input[$cont]['volumen_muestra'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('R' . $i, $input[$cont]['mgl'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('S' . $i, $input[$cont]['hr'] ?? 'N/A');
            $spreadsheet->getActiveSheet()->setCellValue('T' . $i, $input[$cont]['centigrados'] ?? 'N/A');
    ...
    public function getEnsayoPrincipal($id)
    {
        ...
        $ensayo = EnsayoSolidosSecos::where('id_muestra', $id)->where('tipo', 'Ensayo')->first();
    ...
    public function estadoEnsayo(Request $request)
    {
        ...
        $ensayo = EnsayoSolidosSecos::find($request['id']);
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Blanco')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Patrón')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
    ...
    public function store(Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $aluminioGeneral = SolidosSecos::orderBy('id', 'desc')->first();

        if ($request['tipo'] == 'Blanco') {

            $datosBlanco = Blancos::where('lc_list_trials_id', 25)->get()->last();
            ...
            if ($result1 > $result2) {
                ...
                $user = Auth::user();
                $cantidadBlancos = EnsayoSolidosSecos::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoSolidosSecos();
                ...
                $ensayo->lc_solidos_secos_id = $aluminioGeneral->id;
                $ensayo->id_muestra = '';
            ...
            if ($request['tipo'] == 'Patrón') {
                ...
                $datosPatron = Patron::where('lc_list_trials_id', 25)->get()->last();
                ...
                if ($result1 > $result2) {
                    ...
                    $user = Auth::user();
                        $cantidadPatrones = EnsayoSolidosSecos::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('tipo', 'Patrón')->count();
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    $blanco = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                    if ($blanco || $blanco == null) {

                        $patron = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();

                        if ($patron || $patron == null) {

                            $arrayTotal['ensayo'] = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->get()->count();
                            ...
                            if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                                ...
                                $centinela = false;
                                $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 25)->get();
                                ...
                                if ($centinela == true) {
                                    ...
                                    $history->observation = "Se realizo el analisis de la fórmula de solidos secos";
                                ...
                                $buscaEnsayo = Ensayo::where('lc_list_trials_id', 25)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                                $año = date('Y');
                                $array = str_split($año);
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoSolidosSecos::where('users_id', $user->id)->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoSolidosSecos::where('users_id', $user->id)->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoSolidosSecos::where('users_id', $user->id)->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de Solidos secos";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
     ...
     * @version 1.0.0
     *
     * @param EnsayoSolidosSecos $request
     *
     * @return Response
     */
    public function getGrafico(Request $request)
    {
        ...
        $blanco = Blancos::where('lc_list_trials_id', 25)->get()->last();
        ...
        $ensayo = SolidosSecos::orderBy('id', 'desc')->first();
        ...
        $patron = Patron::where('lc_list_trials_id', 25)->get()->last();
                ...
                if($item["tipo"]=="Patrón") {
                    ...
                    if($item["tipo"]=="LCM") {
                        ...
                        if($item["tipo"]=="Duplicado") {
                            ...
                            $contDpr++;
                            if($contDpr<=31){
                                if($item["lc_dpr_pr_solidos_secos"]){
                                    foreach ($item["lc_dpr_pr_solidos_secos"] as $value){
        ...
        $e->downloadFileGoogleDrive($fileIdCopy, "Ensayo Solidos secos", "excel", true);
    }
