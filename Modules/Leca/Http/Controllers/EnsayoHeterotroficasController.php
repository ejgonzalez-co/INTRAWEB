...
use Modules\Leca\Models\PozosPositivo;
...
class EnsayoHeterotroficasController extends AppBaseController
{
    ...
    {
        return view('leca::EnsayoHeterotroficas.index');
    }

    public function getDatosBlanco()
    {

        $datosBlanco = BacteriasHeterotroficas::orderBy('id', 'desc')->first();
    ...
    public function getDatosSiembra()
    {
        if (!Auth::user()->hasRole('Administrador Leca')) {
            ...
            $allEnsayos = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Ensayo')->where('ensayo', 'Heterotroficas')->where('estado', 'Pendiente')->latest()->get();

        } else {

            $allEnsayos = EnsayoMicro::where('tipo', 'Ensayo')->where('ensayo', 'Heterotroficas')->where('estado', 'Pendiente')->latest()->get();
        }
    ...
    public function getResulDosis(Request $request)
    {
        $datosPozos = PozosPositivo::select('resultado_mpn')->where('pozos', $request['p_positivos'])->get()->toArray();
        if($datosPozos[0]['resultado_mpn'] == '<2'){
            $datosPozos = array(
                array(
                    "resultado_mpn" => "0"
                )
    ...
    public function update($id, Request $request)
    {
        $user = Auth::user();
        $Year = date("Y");
        $coliformeGeneral = BacteriasHeterotroficas::orderBy('id', 'desc')->first();
        $user = Auth::user();

        $blanco = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('tipo', 'Blanco')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Si')->get()->last();
        ...
        if ($blanco) {
            ...
            if ($blanco) {
                ...
                $arrayTotal['ensayo'] = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->get()->count();
                ...
                if ($arrayTotal['ensayo'] < $blancoGeneral->periodicidad && $arrayTotal['ensayo'] < $patronGeneral->periodicidad) {
                    ...
                    $centinela = false;
                    $verifica = Ensayo::where('lc_sample_taking_id', $request['idMuestra']['id'])->where('lc_list_trials_id', "!=", 23)->get();
                    ...
                    if ($centinela == true) {
                        ...
                        $history->observation = "Se realizo el analisis de la fórmula de Heterotroficas";
                    ...
                    $buscaEnsayo = Ensayo::where('lc_list_trials_id', 23)->where('lc_sample_taking_id', $request['idMuestra']['id'])->first();
                    $año = date('Y');
                    $array = str_split($año);
                    ...
                    $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_sample_taking_id', $request['idMuestra']['id'])->where('estado', 'Activo')->where('lc_list_trials_id', 23)->first();
     ...
     * @return Response
     */
    public function getAllHeterotroficas()
    {

        if (!Auth::user()->hasRole('Administrador Leca')) {
            $user = Auth::user();

            // $allEnsayos = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('tipo', 'Ensayo')->where('estado', "!=", 'Pendiente')->orWhere('tipo', 'Blanco')->orWhere('tipo', 'Duplicado')->latest()->get();

            $allEnsayos = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->latest()->get();

        } else {
            // $allEnsayos = EnsayoMicro::where('tipo', 'Ensayo')->where('ensayo', 'Heterotroficas')->where('estado', "!=", 'Pendiente')->orWhere('tipo', 'Blanco')->orWhere('tipo', 'Duplicado')->latest()->get();

            $allEnsayos = EnsayoMicro::where('ensayo', 'Heterotroficas')->latest()->get();
        }
    ...
    public function indexEjecutar()
    {
        ...
        $blanco = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('ensayo', 'Heterotroficas')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('aprobacion_usuario', 'Si')->get()->last();

        if ($blanco) {

            $patron = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('tipo', 'Patrón')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where('aprobacion_usuario', 'Si')->get()->last();
            ...
            if ($blanco) {
                ...
                if ($arrayTotal['ensayo'] <= $blancoGeneral->periodicidad && $arrayTotal['ensayo'] <= $patronGeneral->periodicidad) {
                    ...
                    return view('leca::EnsayoHeterotroficas.ejecutar');
    ...
    public function allMuestras()
    {
        ...
        $ensayo = Ensayo::where('lc_list_trials_id', 23)->where('estado', 'Pendiente')->get();
    ...
    public function storeTendido(Request $request)
    {
        ...
        $muestras = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 23)->where('estado', 'Activo')->get();
    ...
    public function allTendido()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 23)->where('estado', 'Activo')->get();
    ...
    public function allTendidoFinalizado()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $tendido = TendidoMuestras::where('users_id', $user->id)->where('lc_list_trials_id', 23)->whereDate('fecha_finalizado', '=', Carbon::now()->format('Y-m-d'))->where('estado', 'Finalizado')->get();
    ...
    public function consultaDecimalesBlanco()
    {
        ...
        $user = Auth::user();
        $blancoDecimales = Blancos::where('lc_list_trials_id', 23)->get()->last();
    ...
    public function consultaDecimalesPatron()
    {
        ...
        $user = Auth::user();
        $patronDecimales = Patron::where('lc_list_trials_id', 23)->get()->last();
    ...
    public function consultaDecimalesEnsayo()
    {
        ...
        $user = Auth::user();
        $arrayTotal = [];
        $ensayoDecimales = BacteriasHeterotroficas::where('lc_list_trials_id', 23)->get()->last();
    ...
    public function exporta(Request $request)
    {
        ...
        $inputFileType = 'Xlsx';
        $inputFileName = storage_path('app/public/leca/excel/Heterotroficas.xlsx');
        ...
        for ($i = 15; $i < $numTotal; $i++) {
            ...
            $pozos = $input[$cont]['pozos_grandes'];
            if($pozos == null){
                $pozos = "N/A";
            }
            ...
            $spreadsheet->getActiveSheet()->setCellValue('K' . $i, $dia);
            ...
            $spreadsheet->getActiveSheet()->setCellValue('M' . $i, $pozos ?? 'N/A');
    ...
    public function getCondicionEnsayos()
    {
        ...
        $blanco = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Blanco')->where('ensayo', 'Heterotroficas')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where(function ($query) {
        ...
        if ($blanco) {
            ...
            if ($blanco->aprobacion_usuario == 'Si') {
                ...
                $patron = EnsayoMicro::where('users_id', $user->id)->where('tipo', 'Patrón')->where('ensayo', 'Heterotroficas')->where('estado', 'Si')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $blanco->created_at)->where(function ($query) {
                    $query->where('aprobacion_usuario', 'Pendiente')->orwhere('aprobacion_usuario', 'Si');
                })->get()->last();
            } else {
                $patron = null;
            ...
            if ($patron != null) {
                ...
                if ($patron->aprobacion_usuario == 'Si') {
                    $arrayTotal['ensayo'] = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('tipo', 'Ensayo')->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->whereTime('created_at', '>=', $patron->created_at)->get()->count();
                } else {
    ...
    public function store(Request $request)
    {
        ...
        if ($request['tipo'] == 'Blanco') {
            ...
            $datosBlanco = Blancos::where('lc_list_trials_id', 23)->get()->last();
            $result1 = floatval($datosBlanco->lcm);
            $result2 = floatval($request['resultado_general']);

            if ($result1 >= $result2) {
                $user = Auth::user();
                $cantidadBlancos = EnsayoMicro::where('users_id', $user->id)->whereDate('created_at', '=', Carbon::now()->format('Y-m-d'))->where('ensayo', 'Heterotroficas')->where('tipo', 'Blanco')->count();
                $ensayo = new EnsayoMicro();
                $ensayo->estado = 'Si';
                $ensayo->aprobacion_usuario = 'Pendiente';
                $ensayo->ensayo = 'Heterotroficas';
                $ensayo->resultado = $request['resultado'];
                $ensayo->pozos_grandes = $request['p_positivos'];
                ...
                if ($request['tipo'] == 'Ensayo') {
                    ...
                    if ($request['tipo'] == 'Duplicado') {
                        ...
                        if ($request['metodo'] == "Duplicado") {
                            $duplicado = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('tipo', 'Duplicado')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                            if ($duplicado >= 1) {
                            ...
                            if ($request['metodo'] == "Repetición") {
                                $duplicado = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('metodo', 'Repetición')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                                if ($duplicado >= 1) {
                                ...
                                if ($request['metodo'] == "Regla de decisión") {
                                    ...
                                    $duplicado = EnsayoMicro::where('users_id', $user->id)->where('ensayo', 'Heterotroficas')->where('metodo', 'Regla de decisión')->where('id_muestra', $request['idMuestra']['id_muestra'])->get()->count();
                        ...
                        $history->observation = "Se realizo duplicado a la muestra de Heterotroficas";
                        $history->lc_sample_taking_id = $request['idMuestra']['id'];
                        $history->save();
    ...
    public function getGrafico(Request $request)
    {
        ...
        $data[] = ["range" => "Heterotroficas!B7", "values" => [["Bacterias heterotróficas"]]];

        $fechaActual = date('d-m-Y');
        $anio = date("y", strtotime($fechaActual));
        $mes = date("m", strtotime($fechaActual));
        
        $data[] = ["range" => "Heterotroficas!V8", "values" => [[$mes]]];
        $data[] = ["range" => "Heterotroficas!V7", "values" => [[$anio]]];
        ...
        if(count($arrayBk)>0){
            $data[] = ["range" => "Heterotroficas!B28", "values" => [$arrayBk]];
            $data[] = ["range" => "Heterotroficas!B29", "values" => [$arrayBkF]];
            $data[] = ["range" => "Heterotroficas!B30", "values" => [$arrayBkC]];
        }

        if(count($arrayBk2)>0){
            $data[] = ["range" => "Heterotroficas!B33", "values" => [$arrayBk2]];
            $data[] = ["range" => "Heterotroficas!B34", "values" => [$arrayBk2F]];
            $data[] = ["range" => "Heterotroficas!B35", "values" => [$arrayBk2C]];
        }

        $fileIdCopy = $e->editFileExcelBatch("1kbDj4whimlpGach2JBh12AnXhY_cWpGgtMuLY0KdKiw", $data, true);
