                    ...
                    <thead>
                        <tr>
                            ...
                            <th  style="background-color: #E9ECEF"
                                v-for="(option, key) in nameColumnOptions"
                                :key="key"
                            >
                                {{ option.label }}
                            </th>
                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, key) in getDataColumValue" :key="key">
                            <td v-for="(col, k) in nameColumnOptions" :key="k">
                                <p v-if="col.name == 'code_cost'"> {{ row[col.name] }}</p>
                                <p v-if="col.name == 'name'"> {{ row[col.name] }}</p>  
                                <p v-if="col.name == 'cost_center'"> {{ row[col.name] }}</p>
                                <p v-if="col.name == 'cost_center_name'"> {{ row[col.name] }}</p>

                                <div v-if="update" class="" >
                                    <currency-input v-if="col.name == 'value_eddition'"
                                        id='value_eddition'
                                        name="value_eddition" 
                                        v-model="$parent['dataForm']['adding_cost_item'][key]['value_addition']"
                                        :currency="{'prefix': '$ '}"
                                        locale="es"
                                        class="form-control"
                                        :key="keyRefresh"
                                        @change="updateSumOfValuesAdition"
                                        >
                                    </currency-input>

                                    <input-operation-change v-if="col.name == 'total_eddition'"
                                        :precision = 2
                                        name-field="total_total"
                                        :number-one="$parent['dataForm']['adding_cost_item'][key]['value_addition']"
                                        :number-two="$parent['dataForm']['adding_cost_item'][key]['value_item_old']"
                                        :value="$parent['dataForm']" 
                                        :key="$parent['dataForm']['adding_cost_item'][key]['value_eddition'] + keyRefresh"                   
                                        operation="suma"
                                        prefix='$ '
                                    ></input-operation-change>

                                    <p v-if="col.name == 'value_item_old' "> {{ formatCurrency(row[col.name]) }}</p>
                                </div>
                                ...
                                <div v-else class="">
                                    ...
                                    <currency-input v-if="col.name == 'value_eddition'"
                                        id='value_eddition'
                                        name="value_eddition" 
                                        v-model="$parent['dataForm']['rubrosItems'][key]['value_eddition']"
                                        :currency="{'prefix': '$ '}"
                                        locale="es"
                                        class="form-control"
                                        :key="keyRefresh"
                                    ...
                                    <input-operation-change v-if="col.name == 'total_total'"
                                        :precision = 2
                                        name-field="total_total"
                                        :number-one="$parent['dataForm']['rubrosItems'][key]['value_eddition']"
                                        :number-two="$parent['dataForm']['rubrosItems'][key]['value_item']"
                                        :value="$parent['dataForm']" 
                                        :key="$parent['dataForm']['rubrosItems'][key]['value_eddition'] + keyRefresh"                   
                                        operation="suma"
                                        prefix='$ '
                                    ></input-operation-change>

                                    <p v-if="col.name == 'value_item' && update == false"> {{ formatCurrency(row[col.name]) }}</p>
...
export default class DynamicListComponent extends Vue {
    ...
    /**
     * Valida si se debe recomputarizar
     */
    @Prop({ type: Boolean, default: false })
    public isRecompute: boolean;

    /**
     * Valida si se debe eliminar elemento de la lista
     */
    ...
    public sumOfValuesAdition: number;
    ...
    public addToList(): void {
        let dataList1 = utility.clone(this.dataForm);
        ...
        this.recomputeDataColumns();
        this.addedToListEvent();
        this.$forceUpdate();
        this.clean();
    }
     ...
     *
     *
     * @author Jhoan Sebastian Chilito S. - Marz. 02 - 2021
     * @version 1.0.0
     */
    public addedToListEvent() {
        this.$emit("addedToListEvent", this.dataList);
    }
    ...
    /**
     * Recomputarizar de una propiedad computada de un componente por medio del nombre de la propiedad
     *
     * @author Jhoan Sebastian Chilito S. - Marz. 01 - 2021
     * @version 1.0.0
     */
    public recomputeDataColumns() {
        // Valida si se recomputarizar la propiedad computada
        if (this.isRecompute) {
            (this as any).$recompute("getDataColumValue");
        }
    }
                    ...
                    await new Promise(resolve =>
                        ...
                        if (selectRef) {
                            ...
                            if (colOption.selectItem) {
                                let datoColOption = data[colOption.name].map(
                                    Number
                                );
     ...
     *
     *
     * @author Nicolas Ortiz. - Agosto. 18 - 2022
     * @version 1.0.0
     */
    public clean() {
        this.dataForm = {};
