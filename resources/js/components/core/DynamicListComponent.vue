                    ...
                    <tbody>
                        ...
                        <tr v-for="(row, key) in getDataColumValue" :key="key">
                            <td v-for="(col, k) in nameColumnOptions" :key="k">
                                <span v-if="!col.isEditable">{{ col.nameObjectKey ? (col.nameObjectKey.length > 1 ? (row[col.nameObjectKey[0]] ? row[col.nameObjectKey[0]][col.nameObjectKey[1]] : row[col.name]) : (row[col.nameObjectKey[0]] instanceof Array ? row[col.nameObjectKey[0]]?.join(", ") : row[col.name])) : row[col.name] === true ? "Si" : (row[col.name] === false ? "No" : row[col.name]) }}</span>
...
export default class DynamicListComponent extends Vue {
    ...
    public vaciarAutocompleteQuery() {
        this.nameColumnOptions.forEach(async (colOption) => {
            // Valida que sea una lista referida
            if (colOption.refList) {
                (this.componentePadre.$refs[colOption.refList] && this.componentePadre.$refs[colOption.refList]["query"]) ? this.componentePadre.$refs[colOption.refList]["query"] = "" : null;
            }
        });
    }
